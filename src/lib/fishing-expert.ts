...
export function getSmartFishingTips(
  ...
  if (isStormy) {
    ...
    weatherTip = timePeriod === 'morning'
      ...
      : timePeriod === 'evening'
      ? 'Горещо време — вечерната прохлада раздвижва рибата тази вечер.'
      : timePeriod === 'night'
      ? 'Горещо време — нощта е най-добрият момент, рибата е на повърхността.'
      : 'Горещо време (над 25°C) — рибата е на дълбочина през деня. Чакай вечерта.';
  } else if (isCold) {
    weatherTip = 'Студено време — рибата е по-бавна. Използвайте бавни презентации и малки стръвки.';
  } else if (isRainy) {
    weatherTip = terrain === 'river'
      ? `Дъждовно — мътната вода и кислородът активират рибата ${terrainLabel}. Отличен момент за риболов ${timeLabel}!`
      : `Дъждовно — дъждът насища водата с кислород ${terrainLabel}. Отличен момент за риболов ${timeLabel}!`;
  } else {
