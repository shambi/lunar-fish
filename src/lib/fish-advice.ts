...
export function getDailyAdvice(
  ...
  if (name === 'Сом') {
    ...
    s2 = terrain === 'river'
      ...
      : `В язовира търси мряната по дъното близо до каменисти участъци с червей и по-тежко олово.`;
  }
  else if (name === 'Костур') {
    s2 = terrain === 'river'
      ? `Води малък туистер (3-5см) срещу течението с агресивни потрепвания — хапката е рязка.`
      : `В язовира търси костура по ръба на дълбочината с джиг монтаж и бяло/жълто при мътна вода.`;
  }
  else if (name === 'Платика') {
    s2 = terrain === 'river'
      ? `Лови с фидер на течение — 40-50г кошница, червей или царевица, повод 40-60см.`
      : `В язовира заложи на метод фийдър с царевица или пелети — търси платиката край тръстиката.`;
  }
  else if (name === 'Бабушка') {
    s2 = terrain === 'river'
      ? `Лови на ваглер в по-тихите участъци на реката с малък опариш — хапката е деликатна.`
      : `В язовира търси бабушката сутрин край повърхността с ваглер и опариш № 18 кука.`;
  }
  else if (name === 'Червеноперка') {
    s2 = terrain === 'river'
      ? `Лови с поплавък в затишията и заливите на реката — хляб и тесто работят отлично.`
      : `В язовира търси червеноперката в тръстиката с червей или хляб на лек поплавъчен монтаж.`;
  }
  else if (name === 'Дъгова пъстърва') {
    s2 = terrain === 'river'
      ? `Води воблер срещу течението с кратки рипки — пъстървата атакува рязко и бързо.`
      : `В платен язовир хвърли спинер № 2-3 или паста на въдица — дъговата атакува агресивно.`;
  }
  else if (name === 'Бибан') {
    s2 = terrain === 'river'
      ? `Лови с малък джиг или туистер (5-7см) в затишията — биванът е агресивен но внимателен.`
      : `В язовира търси бибана с джиг монтаж край дъното — активен е целогодишно.`;
  }
  else if (name === 'Скобар') {
    if (terrain === 'river') {
      ...
      s2 = `В язовира скобарът е рядкост — търси го по каменистото дъно с червей.`;
    }
  }
  else if (name === 'Кефал') {
    s2 = terrain === 'river'
      ? `Търси го в бързеите с хляб или тесто — кефалът обича кислородна вода.`
      : `В язовира кефалът е рядък — опитай с хляб край брега в топли дни.`;
  }
  ...
  // Generic fallback based on terrain
    ...
    if (isPredator) {
      ...
      if (temp < 12) {
        s2 = `В студена вода на язовир/езеро дръж стръвта близо до дъното и захранвай на малки порции с по-фина смес.`;
      } else {
        s2 = `На по-топъл язовир/езеро търси ${name} по ръба на плиткото и храни по-ритмично с царевица или пелети на пружина.`;
      }
    }
  } else {
    // River
