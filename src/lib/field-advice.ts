...
export function generateFieldAdvice(params: FieldAdviceParams): string {
  ...
  if (isStrongWind) {
    ...
    s2 = needsDarkLeader
      ? `Водата е мътна — рибата разчита на мирис, използвай по-ароматна стръв, но поводът остава тъмен/незабележим и целѝ тихите зони зад препятствия, не средата на течението; препоръчителна кука ${recommendedHookSize}, влакно ${recommendedLineThickness}.`
      : `Водата е мътна — рибата разчита на мирис, използвай сигнално влакно и по-ароматна стръв, но целѝ тихите зони зад препятствия, не средата на течението; препоръчителна кука ${recommendedHookSize}, влакно ${recommendedLineThickness}.`;
  } else if (isClearSunDay) {
    s1 = `Небето е ясно и слънцето грее силно над водата.`;
    s2 = `При ярко слънце рибата вижда по-добре менящите се цветове на влакното и търси по-хладни, по-дълбоки слоеве — избягвай крещящи цветове; препоръчителна кука ${recommendedHookSize}, влакно ${recommendedLineThickness}.`;
  } else if (isClearCalm) {
    s1 = `Водата е бистра, а вятърът е слаб — рибата вижда добре какво се случва на брега.`;
    s2 = `Водата е бистра и тихо — рибата вижда риболовеца ясно, затова използвай по-тънък повод, фино поднасяне и минимум движение край брега; препоръчителна кука ${recommendedHookSize}, влакно ${recommendedLineThickness}.`;
  } else {
